    ...
    $.ajax({
        ...
        success: function(json2){
            ...
            $.ajax({
                ...
                if(response.length !==0){   //Record found in quiz logs
                    if(response[0].quizcompleted == 'false'){
                        ...
                        if(typeof json2['maxTimeToFinish']!=='undefined' && json2['maxTimeToFinish']!==null){
                            if(json2['maxTimeToFinish']<Seconds_Between_Dates){
                                $('.quizstartpage').removeClass('hidden');
                                /* ------------------- Quiz Landing page HTML ---------------- */
                                $('.startpagehtml').html('Quiz Time finished. <br> Please click on <b>\'Review Quiz\'</b> button to review your score.'); //Updating HTML for Quiz Landing Page card-text (You are about to start the quiz.....)
                    ...
                    else{   //Quiz completed already
                        // alert('2');
                        $('.quizstartpage').removeClass('hidden');
                        /* ------------------- Quiz Landing page HTML ---------------- */
                        $('.startpagehtml').html('You have finished the quiz. <br> Please click on <b>\'Review Quiz\'</b> button to review your score.'); //Updating HTML for Quiz Landing Page card-text (You are about to start the quiz.....)
                        //Updating the above startpagehtml HTML further for quiz minutes and quiz total questions
                        $('.quiztitle').html(json2.quiz_title); //Updating HTML for Quiz Landing page card-title
                        $('.startquizbutton').html('Review Quiz');
                        /* ------------------- Quiz Landing page HTML ---------------- */
                ...
                else{   //No record found in quiz logs
            ...
            $('.startquizbutton').off('click').on('click', function () {
                ...
                $.ajax({
                    ...
                    function quizStartLogic(startedAt, que_no, quizAnswers2, questionCorrectIncorrect2, quizFinished){
                        // alert('hello');
                        ...
                        if(typeof json2['maxTimeToFinish']!=='undefined' && json2['maxTimeToFinish']!==null) {
                            var countDownDate = addMinutes(quizStartedAt, json2['maxTimeToFinish']).getTime();
                            ...
                            var x = setInterval(function() {
                                ...
                                document.getElementById("demo").innerHTML = /*days + "d " +*/"Time Left: " +hours + "h "
                                    + minutes + "m " + seconds + "s ";

                                // If the count down is over, write some text
                                if (distance < 0) {
                                    clearInterval(x);
                                    document.getElementById("demo").innerHTML = "Quiz Time Finished!";
                                    // alert('hello');
                                    quizCompleted = true;
                                    ...
                                    if(questionCorrectIncorrect[queNo] == 'correct'){
                                        ...
                                        $('.labelexplanation').html(json2.pages[queNo]['questions'][0]['explanation']);
                                        ...
                                        if(json2.pages[queNo]['questions'][0]['type']=='checkbox'){
                                            $('.correctanswer').html(options2(json2.pages[queNo]['questions'][0]['correctAnswer']));
                                        }
                                        else{
                                            $('.correctanswer').html(options(json2.pages[queNo]['questions'][0]['correctAnswer']));
                                        }
                                    }
                                    $('.quecompletebutton').attr('disabled', 'disabled');
                                    var score = 0;
                        ...
                        $('.progressbartext').css('width', queNo*100/(json2.pages.length - 1)+'%');
                        quizStarted = true;
                        quizCompleted = false;
                        if(typeof json2['maxTimeToFinish']!=='undefined' && json2['maxTimeToFinish']!==null){
                            if(json2['maxTimeToFinish']<Seconds_Between_Dates){
                        ...
                        $('.quizquestion').html(queNo+'. '+json2.pages[1]['questions'][0]['title']); //Updating question in Quiz question page
                        $('.currentquestion').html(queNo); //Updating Current Question to 1 in Progress Bar

                        if(json2.pages.length - 1 == 1){  //If Quiz has only 1 question disable previous and next button
                            $('.queprevbutton').attr('disabled', 'disabled');
                            $('.quenextbutton').hide();
                        }
                        else if(quizCompleted == true && json2.pages.length - 1 == queNo){
                            $('.quenextbutton').hide();
                            $('.quecompletebutton').addClass('hidden');
                        }
                        else{ //Otherwise just disable previous button as there are more questions and we are at question number 1. Also hide Completed button
                            $('.quecompletebutton').addClass('hidden');
                        ...
                        if(json2.pages[que_no]['questions'][0]['type']=='checkbox'){ //If the question type is MCQ More than 1 correct
                            ...
                            for(var i = 0; i< json2.pages[que_no]['questions'][0]['choices'].length; i++){
                                if(json2.pages[que_no]['questions'][0]['choices'][i] && json2.pages[que_no]['questions'][0]['choices'][i].trim()!==''){
                                    if(typeof quizAnswers[queNo] !=='undefined'){ //Means that the user has already filled the response of the question before and he is right now just reviewing the quiz. So we need to check the checkbox he selected originally
                                        var myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
                                            '                  <input style="margin-left: 0!important;" type="checkbox" id="checkbox" />'+
                                            '                  <label for="checkbox">'+json2.pages[queNo]['questions'][0]['choices'][i]+
                                            '                  </label>'+
                                            '                </div>'+
                                            '              </li>';
                                        for(var j =0; j < quizAnswers[queNo].length; j++){  //Checking if this option was checked by user, if yes then add checked attribute to it
                                    ...
                                    else{ //User would mark the answer first time
                                        var myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
                                            '                  <input value="'+i+'" style="margin-left: 0!important;" type="checkbox" id="checkbox'+i+'" />'+
                                            '                  <label for="checkbox'+i+'">'+json2.pages[que_no]['questions'][0]['choices'][i]+
                                            '                  </label>'+
                                            '                </div>'+
                                            '              </li>';
                                    }
                            ...
                            $('.checkbox input').off('change').on('change', function(){
                                var selected = [];
                                var j = 0;
                        ...
                        else if(json2.pages[que_no]['questions'][0]['type']=='radiogroup'){ //If the question type is MCQ Only 1 correct
                            ...
                            for(var i = 0; i< json2.pages[que_no]['questions'][0]['choices'].length; i++){
                                if(json2.pages[que_no]['questions'][0]['choices'][i] && json2.pages[que_no]['questions'][0]['choices'][i].trim()!==''){
                                    var myvar;
                                    if(typeof quizAnswers[queNo] !=='undefined' && quizAnswers[queNo] == i){
                                        myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
                                            '                  <input checked value="'+i+'" type="radio" name="radio" id="radio'+i+'" />'+
                                            '                  <label for="radio'+i+'">'+
                                        ...
                                        myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
                                            '                  <input value="'+i+'" type="radio" name="radio" id="radio'+i+'" />'+
                                            '                  <label for="radio'+i+'">'+
                                            json2.pages[que_no]['questions'][0]['choices'][i]+
                                            '                  </label>'+
                                            '                </div>'+
                            ...
                            $('.checkboxes').html(html);
                            $('input:radio[name="radio"]').off('change').on('change',function(){
                    ...
                    if(response.length !==0){   //Record found in quiz logs
                        if(response[0].quizcompleted == 'false'){
                            ...
                            if(typeof response[0].questionCorrectIncorrect!=='undefined'){
                                questionCorrectIncorrect2 = JSON.parse(response[0].questionCorrectIncorrect);
                            }
                            else{   //If started quiz but not even filled first question response
                            ...
                            quizStartLogic(response[0].date, que_no, quizAnswers2, questionCorrectIncorrect2, 'false');
                            ...
                            quizStartLogic(response[0].date, que_no, quizAnswers2, questionCorrectIncorrect2, 'true');
                            $('.quecompletebutton').removeAttr('disabled');
                            //Quiz finished
                        }
                    }
            ...
            $('.quenextbutton').off('click').on('click', function () {
                $('.quizstartpage').addClass('hidden'); //hiding landing page of quiz
                $('.quizquestionpage').removeClass('hidden'); //and showing question page of quiz
                ...
                if(json2.pages[queNo]['questions'][0]['type']=='checkbox'){
                    ...
                    if(selected.length == 0){ //If no checkbox is selected in MCQ more than one correct
                        if(timeOver == false){
                            $('.requiredmessage').removeClass('hidden');
                            logicToExecute = -1;
                        }
                    ...
                    else{ //means no radio box is checked
                        if(timeOver == false){
                            $('.requiredmessage').removeClass('hidden');
                            logicToExecute = -1;
                        }
                ...
                if(logicToExecute == 1){
                    ...
                    var totalQuestions = json2.pages.length - 1;  //Total question in Quiz
                    if(queNo < totalQuestions){ //User is not on last question, so enable both Previous and Next buttons
                        $('.quecompletebutton').addClass('hidden');
                        if(timeOver){
                            $('.queprevbutton').removeAttr('disabled');
                        }
                    ...
                    else{ //User is on last question, so show Complete button and disable Next button and enable Previous button
                        $('.quenextbutton').hide();
                        if(quizCompleted == true){
                            $('.quecompletebutton').attr('disabled', 'disabled');
                            $('.nextbuttonquiz').removeClass('hidden');
                    ...
                    if(queNo < totalQuestions || queNo == totalQuestions){  //The user is on a question
                        $('.currentquestion').html(queNo);  //Updating current question in progressbar
                        $('.quizquestion').html(queNo+'. '+json2.pages[queNo]['questions'][0]['title']);  //Updating question in Quiz question page
                        if(json2.pages[queNo]['questions'][0]['type']=='checkbox'){


                            /*-------------------------Making HTML of choices of question number 1  --------------------*/
                            var html = '';
                            for(var i = 0; i< json2.pages[queNo]['questions'][0]['choices'].length; i++){ //Iterating through all the choices of this MCQ

                                if(json2.pages[queNo]['questions'][0]['choices'][i] && json2.pages[queNo]['questions'][0]['choices'][i].trim()!==''){
                                    if(typeof quizAnswers[queNo] !=='undefined'){ //Means that the user has already filled the response of the question before and he is right now just reviewing the quiz. So we need to check the checkbox he selected originally
                                        var myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
                                            '                  <input style="margin-left: 0!important;" type="checkbox" id="checkbox" />'+
                        ...
                        else if(json2.pages[queNo]['questions'][0]['type']=='radiogroup'){
                            var html = '';
                            for(var i = 0; i< json2.pages[queNo]['questions'][0]['choices'].length; i++){
                                if(json2.pages[queNo]['questions'][0]['choices'][i] && json2.pages[queNo]['questions'][0]['choices'][i].trim()!==''){
                                    if(typeof quizAnswers[queNo] !=='undefined' && quizAnswers[queNo] == i){
                                        var myvar = '<li class = "list-group-item">'+
                                            '                <div class="checkbox">'+
            ...
            $('.queprevbutton').off('click').on('click', function () {
                ...
                if(json2.pages[queNo]['questions'][0]['type']=='checkbox'){
                    ...
                    $('.progressbartext').attr('aria-valuenow', progress).css('width',progress+'%');
                    var html = '';
                    for(var i = 0; i< json2.pages[queNo]['questions'][0]['choices'].length; i++){
                        if(json2.pages[queNo]['questions'][0]['choices'][i] && json2.pages[queNo]['questions'][0]['choices'][i].trim()!=='') {
                            var myvar = '<li class = "list-group-item">' +
                    ...
                    $('.currentquestion').html(queNo);
                    $('.quizquestion').html(queNo+'. '+json2.pages[queNo]['questions'][0]['title']);
                }
                else if(json2.pages[queNo]['questions'][0]['type']=='radiogroup'){
                    var totalQuestions = json2.pages.length - 1;
                    var progress = queNo*100/totalQuestions;
