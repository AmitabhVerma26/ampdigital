...
router.get("/accomplishments/:webinarid/:userid", function (req, res) {
  try {
    ...
    webinaree.findOne({ _id: req.params.userid }, function (err, user) {
      if (user) {
        webinar.findOne(
          { deleted: { $ne: "true" }, _id: req.params.webinarid },
          function (err, webinarDoc) {
            if (webinarDoc) {
              if (req.isAuthenticated()) {
...
router.get("/upcoming", function (req, res) {
  try {
    ...
    webinar.find(
      { deleted: { $ne: "true" }, date: { $gte: new Date() } },
      null,
      { sort: { date: -1 } },
      function (err, webinars) {
        if (req.isAuthenticated()) {
...
router.get("/concluded", function (req, res) {
  try {
    ...
    webinar.find(
      { deleted: { $ne: "true" }, date: { $lte: new Date() } },
      null,
      { sort: { date: -1 } },
      function (err, webinars) {
        if (req.isAuthenticated()) {
...
router.get("/thankyoupage/:webinarurl", function (req, res) {
  try {
    ...
    webinar.findOne(
      { deleted: { $ne: true }, webinarurl: req.params.webinarurl },
      function (err, webinarDoc) {
        if (req.isAuthenticated()) {
...
router.get("/iframe", isAdmin, function (req, res) {
  webinar.find({ deleted: { $ne: true } }, function (err, webinars) {
    if (req.isAuthenticated()) {
      res.render("adminpanel/webinariframe", {
        moment: moment,
        webinars: webinars,
        title: "Express",
        email: req.user.email,
...
router.get("/attendees/datatable", function (req, res) {
  ...
  if (req.query.sSearch != "") {
    var arr = [
      { firstname: { $regex: "" + req.query.sSearch + "", $options: "i" } },
      { webinarname: { $regex: "" + req.query.sSearch + "", $options: "i" } },
      { lastname: { $regex: "" + req.query.sSearch + "", $options: "i" } },
                  ...
                  if (courses[h]["_id"] == docs[i].webinarid)
                    accesscourses =
                      ...
                      `<option ${
                        ...
                        courses[h]["webinarname"]
                      }</option>`;
                }
                $row.push(`
                            <form data-certificates="${
                              docs[i].certificates
                            }" data-name="${
                              docs[i]["firstname"] + " " + docs[i]["lastname"]
                            }" data-email="${docs[i].email}" data-id="${
                              docs[i]._id
                            }" data-webinarid="${
                              docs[i].webinarid
                            }" class="addcertificate" action="">
...
router.get("/:webinarurl", function (req, res) {
  ...
  webinar.findOne(
    { deleted: { $ne: true }, webinarurl: req.params.webinarurl },
    function (err, webinar) {
      if (webinar) {
        if (req.isAuthenticated()) {
          res.render("webinars/webinar", {
            title: "Express",
            webinar: webinar,
            moment: moment,
            email: req.user.email,
...
router.post("/add", async function (req, res) {
  try {
    ...
    res
      .status(200)
      .send("Webinar successfully added. Redirecting to manage page...");
  } catch (error) {
    // Handle validation or database error
    res
      .status(400)
      .json({ error: "Invalid webinar details. Please check and try again." });
  }
});
...
router.post("/addwebinaree", async function (req, res) {
  try {
    const termsandconditions =
      req.body.termsandconditions && req.body.termsandconditions === "on";
...
router.delete("/remove", async function (req, res) {
  try {
    ...
    if (result.nModified === 1) {
      ...
      res
        .status(400)
        .json({ error: "Invalid webinar ID. Please check and try again." });
    }
  } catch (error) {
    res
      .status(500)
