...
router.get("/", isLoggedIn, function (req, res) {
  ...
  if (req.user.courses) {
    lmsCourses.find(
      { deleted: { $ne: "true" }, _id: { $in: req.user.courses } },
              ...
              Promise.all(jobQueries).then(function (listOfJobs) {
                Promise.all(jobQueries2).then(function (listOfJobs2) {
                  Promise.all(jobQueries3).then(function (listOfJobs3) {
                    ...
                    for (var i = 0; i < jobQueries.length; i++) {
                      ...
                      for (var j = 0; j < listOfJobs[i].length; j++) {
                        if (listOfJobs[i][j]["element_type"] == "video") {
                          ...
                          if (
                            typeof listOfJobs[i][j]["duration"] !==
                              "undefined" &&
                            listOfJobs[i][j]["duration"]
                          ) {
                            temp =
                              temp +
                              parseInt(listOfJobs[i][j]["duration"]) * 60;
                          }
                        }
                      }
                      modules[i]["modulesVideoLength"] =
                        Math.round(temp / 60) + " minutes ";
                    }
                    var sum = 0;
                    var moduleCnt;
                    for (var i = 0; i < jobQueries.length; i++) {
                      ...
                      modules[i]["percentagecompleted"] = Math.round(
                        ((listOfJobs2[i].length + listOfJobs3[i].length) *
                          100) /
                          listOfJobs[i].length,
                      );
                    ...
                    // res.json(modules);
                    res.render("courses/course_modules", {
                      course: course,
                      avg: avg,
                      title: "Express",
                      modulesVideoLength: modulesVideoLength,
                      modules: modules,
                      moment: moment,
                      email: req.user.email,
                      registered: req.user.courses.length > 0 ? true : false,
      ...
      if (courseobj) {
        ...
        lmsModules.find(
          ...
          function (err, moduleslist) {
            ...
            lmsModules.find(
              { module_id: module_id, deleted: { $ne: "true" } },
              function (err, modules) {
                if (modules.length == 0) {
                  res.redirect("/");
                } else {
                      ...
                      lmsElements.find(
                        ...
                        function (err, elements) {
                          ...
                          lmsModules.findOne(
                            ...
                            function (err, moduleNext) {
                              if (req.isAuthenticated()) {
                                if (moduleNext) {
                                  res.render("courses/course_module", {
                                    ...
                                    email: req.user.email,
                                    registered:
                                      req.user.courses.length > 0
                                        ? true
                                        : false,
                                    recruiter:
                                      req.user.role && req.user.role == "3"
                                        ? true
                                        : false,
                                    user: req.user,
                                  });
